...
A second, **fully independent** tutorial — "the Academy" — lives in `academy_building.schem`, placed at `AcademyBuildingManager.POS = BlockPos(-177,-34,8)`. It deliberately does not reuse or extend the old `tutorial/` package (`TutorialStage`/`TutorialManager`/`NpcDialogue`/`NpcRole`) — a single flat stage enum can't represent 4 independently-progressing NPC rooms, so it's a parallel system in its own `net.necookie.disastersim.academy` package instead. **The Academy is now the default tutorial** (2026-07-13): the main lobby's first button launches it directly (`AcademyManager.startAcademyRun`), and its own certification (Capt. Morfe's `MorfePhase.EVALUATED_PASS`, or a restored `/login` account's `student_accounts.tutorial_completed`) gates the lobby's second button (New Sim Building 2.0), not the old tutorial anymore — see `docs/systems/simulation.md`'s "Lobby Buttons & the Academy→Simulation Bridge". The old tutorial still functions exactly as before but no longer gates anything; it's reachable via `/bfp old_tutorial` for dev/admin use. `/bfp new_tutorial*` was renamed to `/bfp tutorial*` (the old `/bfp tutorial` moved to `/bfp old_tutorial`) to match — a breaking change for staff muscle memory, called out here deliberately. Full dialogue script, per-room coordinate tables, and a Mermaid flow diagram: `docs/academy_script.md`.

**Player invulnerability (2026-07-19):** `startAcademyRun` sets `player.setInvulnerable(true)` — the scripted tour (Reyes's ignite demo, Santos's earthquake drill, stray fall damage) should never actually hurt a trainee. `SimulationManager.startSimulation` clears it unconditionally the moment a real graded simulation begins (Morfe's post-PASS deploy or a direct lobby-button/dev-command start), and `LobbyManager.routePlayer` resets it to `false` on every login as a safety net, since the `invulnerable` flag persists in player NBT across a relog and every login always lands back in the main lobby, never mid-tutorial.

**Auth system (2026-07-13):** `/register <username> <password> <student_id> <section> <full_name>` now creates a persistent Turso `student_accounts` account (salted `PBKDF2WithHmacSHA256` hash via `session.PasswordHasher`, never plaintext) alongside the existing world-saved registration, so a returning student can `/login <username> <password>` on a later visit and — if their account already shows `tutorial_completed=1` — skip straight to the lobby's simulation button without replaying the Academy. `session.AuthManager` holds the per-station "who's logged in" state (transient, like `DropAndRollManager`'s per-UUID maps) and is what `MorfeRoomManager.runEvaluation`'s PASS branch flips via `AuthManager.markTutorialCompleted`. `/history` (player) and `/bfp user <username>` (instructor) both query the same `sessions.username`-keyed run history. `SessionManager.checkin` also now resets `AcademySavedData` for the incoming station UUID — otherwise a new student at a shared station would inherit whatever Academy certification state the previous student left behind, since `AcademyProgress` is keyed by station UUID, not student identity; a returning student's *real* certification lives on their `student_accounts` row instead, restored on `/login`. **Table name note:** this Turso database is shared with the `BERONG_SMP_WEB` dashboard, which already has its own `users` table for admin/staff logins (different schema) — the account table here is deliberately named `student_accounts`, not `users`, after an initial ship that collided with it and broke every `/register` (see `docs/major_plan.md`'s Phase 6 hotfix note).
...
**Fixed (2026-07-17):** the dialogue sequencer's auto-advance/click-skip timing was still using the
old word-count caption guess (`AcademyManager.ticksFor(String)`, ~0.25s/word, capped at 10s) even
after real audio landed — 69 of 72 lines run longer than that guess (by up to ~8s), so the next
line's `ClientboundStopSoundPacket` was killing most instructor lines mid-sentence, which read as
"audio not loading." `AcademyVoiceDurations` (generated by
`scripts/generate_academy_voice_durations.py` from real `ffprobe` durations, +6-tick buffer) now
backs `AcademyManager.ticksFor(DialogueLine)` for both the auto-advance timer and the click-skip
gate — an instructor always finishes speaking, whether the player waits or mashes interact, before
the sequencer can move on. Lines with no recorded audio (`soundKey == null`, e.g.
`MorfeRoomManager`'s runtime-built debrief lines) still fall back to the word-count estimate.

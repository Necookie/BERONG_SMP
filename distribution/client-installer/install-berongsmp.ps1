...
$NeoForgeInstaller = Join-Path $PayloadDir "neoforge-26.2.0.23-beta-installer.jar"
...
try {
    ...
    # -----------------------------------------------------------------------
    Write-Step "Installing NeoForge 26.2.0.23-beta (this can take a minute)..."
    if (-not (Test-Path $NeoForgeInstaller)) {
        Write-Fail "Missing bundled file: payload\neoforge-26.2.0.23-beta-installer.jar`n`nThis installer package is incomplete - re-download it."
    }
    ...
    Write-Host "  2. In the profile dropdown, select 'NeoForge 26.2.0.23-beta' and click Play"

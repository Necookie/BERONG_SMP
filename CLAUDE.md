...
| Command | Effect |
|---|---|
| `/register <username> <password> <student_id> <section> <full_name>` | Creates a persistent Turso `users` account (salted PBKDF2 hash) and starts a session — same effect as before plus a durable login. Falls back to local-only registration with a warning if Turso is offline. |
| `/login <username> <password>` | Restores a returning student's identity and Academy-certified status (if `users.tutorial_completed=1`) without replaying the tutorial. Rate-limited (5 attempts / 5-minute lockout). |

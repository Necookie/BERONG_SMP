...
| `LightBulbBlock` | Hanging pendant light fixture; no FACING (symmetric). Model: iron ceiling cap + black cord + sea-lantern glass globe. Full-brightness `lightLevel=15` (max vanilla light), `noOcclusion()` for its irregular shape. Vanilla light still decays 1 level per block — to evenly flood a large room, space several fixtures ~12–14 blocks apart rather than relying on one. |

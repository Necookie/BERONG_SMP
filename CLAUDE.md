...
| `TableBlock` | Extendable study/library table — single block, no FACING (symmetric). Model (`table_base.json`): `table_top_oak` tabletop (Y12-15) + `table_leg_wood_dark` four corner-post legs and stretcher bars (Y0-12), leaving a visible open kneehole between the legs, unlike `ComputerTableBlock`'s uniform solid box. `NORTH`/`SOUTH`/`EAST`/`WEST` `BooleanProperty`s (vanilla fence/glass-pane idiom) track same-type neighbours; the multipart blockstate hides a corner apron model (`table_apron.json`) on any connected side so a row of tables reads as one continuous tabletop instead of separate tables — the same visual goal as `WhiteboardBlock`'s `CONNECTED_UP`/`CONNECTED_DOWN`, just on the horizontal axes. Too short to ever satisfy `DuckCoverHoldManager`'s above-the-feet cover check on its own cell — see `DuckCoverHoldManager.hasNearbyTable`. Not flammable-flagged — plain `Block.Properties`. |

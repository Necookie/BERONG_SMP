...
| `SchemLoader` | Implements `StructurePlacer`; parses Sponge Schematic v2/v3 `.schem` files, supports 0–3 CCW 90° rotations (rotates offsets and block states), places blocks, and spawns entities (item frames, paintings) from the `Entities` tag — positions, `Facing` byte, and `TileX/Z` are all rotated to match; existing decoration entities in the footprint are discarded before re-placing to prevent duplicates. **Item frame placement invariant (MC 26.x):** Sponge v3 top-level `Pos` = the entity's own AIR block (not the wall). `Data.Facing` = OUTWARD direction (frame face toward viewer, no `.getOpposite()` needed). `ItemFrame(level, pos, direction)` takes the entity's own block as `pos`; the wall is `pos.relative(direction.getOpposite())`. |
...
| `WhiteboardBlock` | Flat wall-mounted classroom whiteboard; FACING only (extends `HorizontalFacingBlock`). Model: white-concrete board surface + gray frame + light-gray marker tray. Tile 2–3 side-by-side for a wide whiteboard. |
...
| `LightBulbBlock` | Flush-mount LED ceiling panel; no FACING (symmetric). Model: thin 1px-recessed white plastic housing bezel (`led_housing_white.png`) behind a wider cool-white diffuser lens (`led_diffuser_glow.png`, custom textures via `scripts/generate_light_bulb_textures.py`) — the diffuser overhangs the housing by 1px on all sides for a backlit edge-glow look. Flat (`Block.box(1,14,1,15,16,15)`, 2px thick) rather than a bulky hanging globe. Full-brightness `lightLevel=15` (max vanilla light — cannot go higher), `noOcclusion()` for its thin shape. Vanilla light still decays 1 level per block regardless of source color/texture — to evenly flood a large room, space several fixtures ~12–14 blocks apart rather than relying on one. |
...
Block models fall into two tiers:
- **Vanilla-texture models** (furniture: `WhiteboardBlock`, `ToiletBlock`, `SinkBlock`, etc.) — stretch existing `minecraft:block/*` textures (concrete, wood, andesite) over cuboids. Fast to author, but reads as flat colored boxes rather than recognizable objects.
- **Custom-texture models** (`ComputerBlock`, `FireAlarmBlock`, all 20 `block/hazard/*` blocks) — use hand-drawn 16×16 PNGs under `textures/block/` (e.g. `computer_case.png`, `bin_plastic_gray.png`) for a purpose-built look, the same way `computer_block.json` uses `computer_case`/`computer_screen_off`/`computer_keyboard`/`computer_mouse` instead of vanilla blocks.

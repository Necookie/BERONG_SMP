...
| `//copyroom` | Reads the caller's active WorldEdit selection, computes room dimensions/areas/volume, and copies a CSV summary to the **server machine's** OS clipboard. Requires the optional WorldEdit dependency (`worldedit-neoforge`, see `CopyRoomCommand`) to be installed; fails cleanly with a chat error otherwise. OP level 2. |

...
|---|---|
| `BerongSMP` | Mod entry point — a thin bootstrap since the registry extraction. Wires the `registry/Mod*` classes to the mod event bus, registers network payloads and lifecycle listeners, calls `DuckCoverHoldManager.bootstrap()` in `commonSetup` (forces the class load that registers its tick handler), and performs one-time world setup (`onServerStarting`/`onServerStarted`). Still owns `MODID` + `LOGGER`. **2026-07-19:** `onServerStarting` sets the `SPAWN_MONSTERS` gamerule false (blocks hostile natural spawning only — `berongsmp:custom_npc` is `MobCategory.MISC` and is never placed through natural spawning, so it's unaffected). `onServerStarted` pins the overworld `WorldClock` to `ClockTimeMarkers.DAY` via `server.clockManager().moveToTimeMarker(...)` — MC 26.x replaced the old `long dayTime` tick counter with a clock-registry system (same API vanilla's own debug-world bootstrap uses with `NOON`), so `ADVANCE_TIME=false` alone only freezes whatever time the clock already happens to be at; this moves it to day first. |

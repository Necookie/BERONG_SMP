...
- **Drop and Roll + crawl** — pressing the "Drop and Roll" key (default `R`, `KeyMappings.DROP_AND_ROLL`) while on fire sends `DropAndRollPayload` (the mod's first serverbound network payload) to `DropAndRollManager.onDropAndRollRequest`, which knocks 30 ticks (1.5s) off the player's remaining fire ticks per press and opens a 100-tick (5s) "dropped" window during which `MobEffects.SLOWNESS` is continuously refreshed — a stand-in for crawling low instead of walking normally. Repeated presses both keep extinguishing and extend the window. Works globally (not gated to an active session), since a player can catch fire from any hazard. `DropAndRollManager.tick` runs from `SimulationManager.onServerTick` alongside `TutorialManager.tick`.
  ...
  - **Physical cover object**: `DuckCoverHoldManager`/`TutorialManager`'s cover check (`!isAir` at `feet.above(1)` or `feet.above(2)`) was always permissive, but until `TableBlock` was added nothing placeable actually satisfied it — every furniture block (e.g. `ComputerTableBlock`) is a single flat block a player stands *next to*, never *under*. `TableBlock` is the first prop that gives real shelter: see its Key Classes entry below.
...
| `TableBlock` | Extendable study/library table, 2 blocks tall via `HALF` (`DoubleBlockHalf.LOWER`/`UPPER`, same idiom as vanilla tall flowers — `setPlacedBy` places the UPPER half automatically, `playerWillDestroy`/`updateShape` cascade removal of the other half). LOWER's collision is four corner-post legs only (`table_leg_wood_dark`), leaving the centre kneehole walkable; UPPER's tabletop (`table_top_oak`) fills only the bottom half of its block, leaving exactly the headroom a sneaking player's hitbox needs to fit underneath — the first placeable prop that satisfies `DuckCoverHoldManager`/`TutorialManager`'s existing cover check (a player standing in the kneehole already has the tabletop at `feet.above(1)`; no changes needed to either manager). `NORTH`/`SOUTH`/`EAST`/`WEST` `BooleanProperty`s (vanilla fence/glass-pane idiom) track same-half neighbouring `TableBlock`s; the multipart blockstate (`table.json`) hides a corner apron model on any connected side so a row of tables reads as one continuous tabletop instead of separate tables. Not flammable-flagged (unlike `ComputerTableBlock`) — plain `Block.Properties`. |
